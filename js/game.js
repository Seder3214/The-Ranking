...
function getResetGain(layer, useType = null) {
	...
	if (type=="static") {
		...
		return gain.floor().sub(player[layer].points).add(1).max(1).add;
	} else if (type=="normal"){
...
function getNextAt(layer, canMax=false, useType = null) {
	...
	if (type=="static") 
	{
		...
		let amt = player[layer].points.add(player[layer].points.gt(0)?1:0).plus((canMax&&tmp[layer].baseAmount.gte(tmp[layer].nextAt))?tmp[layer].resetGain:0).div(tmp[layer].directMult)
